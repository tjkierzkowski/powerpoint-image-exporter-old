...
class PowerPointImageExporter:
    """All-in-one class to copy out images from a single PowerPoint file into a
    directory"""
    ...
    def __init__(self, pptx_file):
        ...
        if pptx_file_path.suffixes[-1] != ".pptx":
            raise ValueError(
                "Cannot export images from something other than a .pptx file."
                f"The file extension entered was: {''.join(pptx_file_path.suffixes)}"
            )
    ...
    def create_directory_for_images(self, new_directory=None):
        """Create a directory if none exists and skips creation if the directory
        provided already exists. If the new_directory is set to None a ValueError
        is thrown."""
        if new_directory is None:
            raise ValueError(
                "This is not a valid directory path for the images to be placed "
                f"in: {new_directory}"
            )
        image_directory_path = Path(new_directory)
        if image_directory_path.exists() and image_directory_path.is_dir():
            logging.info(
                f"directory '{image_directory_path.resolve()}' already exists... "
                "skipping creation"
            )
        else:
    ...
    def copy_images_to_directory(self):
        ...
        if self.image_directory_path.exists() and self.image_directory_path.is_dir():
            ...
            if len(image_directory_contained_files) != 0:
                raise ValueError(
                    "Will not overwrite the existing image directory at:"
                    f" {self.image_directory_path.resolve()}"
                )
    ...
    def iter_by_shape(
        ...
        """Generator function for returning a Picture Shape or PlaceholderPicture
        along with the slide number and shape
        number in each slide in the powerpoint"""
        if presentation is None:
            raise ValueError(
                f"Presentation {presentation} cannot be traversed to extract "
                "PowerPoint elements"
            )
        if shape_type is None:
            raise ValueError(
                "Please provide a valid Shape enum to traverse your presentation, "
                "see pptx docs for examples"
            )
